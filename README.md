...
| `coderpack` | `gradlew build` | `api-0.100.0.jar` и `api-kotlin-0.100.0.jar` для компиляции модов и `zygote-0.100.0.jar` для JVM. CI отдельно упаковывает сгенерированный агент в релизный файл `agent.zip` |
...
| `build` | `./gradlew build` в каталоге `gradle` | Gradle-плагин, линтер и `coderpack-0.100.0.zip` с командной утилитой |
...
| `idea` | `./gradlew buildPlugin` | `build/distributions/sacred-idea-0.100.0.zip`. При выпуске CI также отправляет плагин в JetBrains Marketplace |
